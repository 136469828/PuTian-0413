...
@property (atomic,strong) UIButton *btnClose;
...
@synthesize progress;
@synthesize size;
@synthesize hasCloseBtn=_hasCloseBtn;
...
- (void)drawRect:(CGRect)rect {
    ...
    if(_hasCloseBtn)
    {
        if(!_btnClose)
        {
            _btnClose=[[UIButton alloc] initWithFrame:CGRectMake(boxRect.origin.x+boxRect.size.width-35, boxRect.origin.y, 35, 35)];
            _btnClose.titleLabel.font=[UIFont boldSystemFontOfSize:19.0];
            [_btnClose setTitle:@"X" forState:UIControlStateNormal];
            [_btnClose addTarget:self action:@selector(closeTouchUp) forControlEvents:UIControlEventTouchUpInside];
...
-(void)closeTouchUp
{
    if([delegate respondsToSelector:@selector(hudClose:)])
    {
        [delegate performSelector:@selector(hudClose:) withObject:self];
    }
}
