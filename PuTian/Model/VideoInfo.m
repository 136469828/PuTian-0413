...
-(id)init
{
    if(self=[super init])
    {
        self.notificationName=kNotificationNameVideoInfo;
    }
    return self;
}
...
-(void)loadData
{
    ...
    {
        self.isLoading1=YES;
        NSString *urlString=[[NSString alloc] initWithFormat:@"%@?m=video&a=getinfo&id=%d&uid=%d",URL_Server,_videoid,_uid];
        NSLog(@"JCK%@",urlString);
...
-(void)comment:(CommentInfo *)comment
{
    if(!self.isLoading2)
    {
        ...
        NSString *urlString=[[NSString alloc] initWithFormat:@"%@?m=system&a=comment&id=%d&uid=%d&stype=%d&gradetype=%d&star=%d&content=%@",URL_Server,_videoid,_uid,3,comment.gradeType,comment.grade,strComment];
        NSURL *url=[[NSURL alloc] initWithString:urlString];
        if(_request1)
        {
            [_request1 setDelegate:nil];
...
-(void)collect:(BOOL)flag
{
    if(!self.isLoading3)
    {
        ...
        NSString *urlString=[[NSString alloc] initWithFormat:@"%@?m=system&a=collection&id=%d&uid=%d&stype=%d&iscollect=%d",URL_Server,_videoid,_uid,3,flag];
        NSURL *url=[[NSURL alloc] initWithString:urlString];
        if(_request3)
        {
            [_request3 setDelegate:nil];
    ...
    if(dic && [[dic objectForKey:@"state"] integerValue]==0)
    {
        ...
        switch(nType)
        {
            case 1:
            {
                ...
                {
                    if(![_linkurl isNotNilOrEmptyString])
                    {
                        if(![[tmpdic objectForKey:@"linkurl"] isNull])
                            self.linkurl=[tmpdic objectForKey:@"linkurl"];
                    }
