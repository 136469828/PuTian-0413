...
@implementation RecreationInfoList
...
-(void)loadDataWithSlideType:(SlideType)slideType keywords:(NSString *)keywords
{
    if(!self.isLoading)
    {
        ...
        {
            urlString=[[NSString alloc] initWithFormat:@"%@?m=article&a=getlist&term_id=%d&uid=%d&attrid=%d&keyword=%@&pindex=%d&psize=%d",URL_Server,3,_uid,_channelid,[keywords stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],tmpPageIndex,_pageSize];
        }
        else
        {
            urlString=[[NSString alloc] initWithFormat:@"%@?m=article&a=getlist&term_id=%d&uid=%d&attrid=%d&pindex=%d&psize=%d",URL_Server,3,_uid,_channelid,tmpPageIndex,_pageSize];
        }
