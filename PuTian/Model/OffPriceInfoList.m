...
@implementation OffPriceInfoList
...
-(void)loadDataWithSlideType:(SlideType)slideType keywords:(NSString *)keywords
{
    if(!self.isLoading)
    {
        ...
        {
            urlString=[[NSString alloc] initWithFormat:@"%@?m=product&a=getlist&pindex=%d&psize=%d&keyword=%@",URL_Server,tmpPageIndex,_pageSize,[keywords stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        }
        else
        {
            urlString=[[NSString alloc] initWithFormat:@"%@?m=product&a=getlist&pindex=%d&psize=%d",URL_Server,tmpPageIndex,_pageSize];
        }
