...
-(void)loadDataWithSlideType:(SlideType)slideType
{
    if(!self.isLoading)
    {
        ...
        if(self.slideType==SlideTypeNormal || self.slideType==SlideTypeDown)
            tmpPageIndex=1;
        NSString *urlString=[[NSString alloc] initWithFormat:@"%@?m=system&a=microshoplist&pindex=1",URL_Server];
//        urlString = [urlString stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding];// 由于含中文字符需要做一个UTF8转码
        ...
        if(tmparr && [tmparr isKindOfClass:[NSArray class]])
        {
            if(tmparr.count>0)
            {
                ...
                for(NSDictionary *tmpdic in tmparr)
                {
                    ...
                    tmpstr=[tmpdic objectForKey:@"shopname"];//名字
                    if([tmpstr isNotNilOrEmptyString])
                        info.title=tmpstr;
                    
                    tmpstr=[tmpdic objectForKey:@"introduce"];//副标题
                    if([tmpstr isNotNilOrEmptyString])
                        info.excerpt=tmpstr;
                    
                    tmpstr=[tmpdic objectForKey:@"picurl"];//图片
                    if([tmpstr isNotNilOrEmptyString])
                    ...
                    tmpstr=[tmpdic objectForKey:@"createtime"];// 创建时间
                    if([tmpstr isNotNilOrEmptyString])
