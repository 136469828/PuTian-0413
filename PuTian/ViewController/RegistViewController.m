...
#define disCountVerifyURL(mobileNum)  [NSString stringWithFormat:@"http://putian.meidp.com/api.php?m=users&a=getVerificationCode&mobile=%@&stype=1",mobileNum]

@interface RegistViewController ()<UIGestureRecognizerDelegate,UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    [btn addTarget:self action:@selector(registTouchUp) forControlEvents:UIControlEventTouchUpInside];
    [scrollView addSubview:btn];
    
    UILabel *lbl=[[UILabel alloc] initWithFrame:CGRectMake(left, btn.y+btn.height+10, width, 30)];
    lbl.font=[UIFont systemFontOfSize:14.0];
    lbl.textColor=[UIColor colorWithWhite:0.5 alpha:1.0];
    lbl.text=@"* 注册代表同意《指尖上的莆田用户协议》";
...
-(void)timerEvent
{
    if(self.countdown>0)
    {
        ...
        if (!_userInfo.isUnion) {
            _txtVerifyCode.text=_userInfo.verifyCode;
        }
    }
    else
    {
...
-(void)verifyCodeTouchUp
{
    ...
    [_userInfo requestVerifyCodeForRegist];
...
-(void)registTouchUp
{
    ...
    _userInfo.loginId=mobile;
    _userInfo.verifyCode=verifyCode;
    _userInfo.pwd=pwd;
    [_userInfo regist];
}
                    ...
                    if(hasNetworkFlags)
                    {
                        ...
                        if([message isNotNilOrEmptyString])
                        {
                            if([message rangeOfString:@"验证码错误"].location!=NSNotFound)
                                hint=@"验证码错误";
                            else if([message rangeOfString:@"已经注册"].location!=NSNotFound)
                                hint=@"手机号码已经被注册咯";
                        }
                        [MBProgressHUDManager showTextWithTitle:hint inView:self.view];
                    }
                    else
                    {
                        [MBProgressHUDManager showTextWithTitle:@"未连接网络" inView:self.view];
