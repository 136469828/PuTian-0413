...
static const CGFloat kHeightOfTopScrollView = 32;
static const CGFloat kHeightOfButton = 28;
static const CGFloat kWidthOfButtonMargin = 3;
static const CGFloat kFontSizeOfTabButton = 14.0f;
static const CGFloat kFontSizeOfButtonWidth = 18.0f;
static const NSUInteger kTagOfRightSideButton = 999;

@implementation SUNSlideSwitchView
@synthesize beginY = beginY_;
...
{
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, beginY_, self.bounds.size.width, kHeightOfTopScrollView-1)];
...
//    _topScrollView.pagingEnabled = YES;
    ...
    UIImageView *lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, beginY_+kHeightOfTopScrollView+1, 320, 1)];
    lineImageView.backgroundColor = [UIColor blackColor];
    lineImageView.alpha = 0.1;
    [self addSubview:lineImageView];
    
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, beginY_+kHeightOfTopScrollView+2, self.bounds.size.width, self.bounds.size.height - kHeightOfTopScrollView-beginY_)];
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI)
    {
        ...
        {
            _rigthSideButton.frame = CGRectMake(self.bounds.size.width - self.rigthSideButton.bounds.size.width, beginY_,
                                                _rigthSideButton.bounds.size.width, _topScrollView.bounds.size.height);
            
            _topScrollView.frame = CGRectMake(0, beginY_,
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++)
    {
        ...
        CGSize textSize = [vc.title boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfButton) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:kFontSizeOfButtonWidth]} context:nil].size;
        ...
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,2,
                                    textSize.width, kHeightOfButton)];
        //计算下一个tab的x偏移量
...
- (void)adjustScrollViewContentX:(UIButton *)sender
{
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x > self.bounds.size.width - (40+kWidthOfButtonMargin+sender.bounds.size.width))
    {
        //向左滚动视图，显示完整tab文字
...
-(void)scrollHandlePan:(UIPanGestureRecognizer*) panParam
{
    ...
    } else if(_rootScrollView.contentOffset.x >= _rootScrollView.contentSize.width - _rootScrollView.bounds.size.width+40)
    {
        if (self.slideSwitchViewDelegate
